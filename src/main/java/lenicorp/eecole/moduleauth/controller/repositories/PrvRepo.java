...
public interface PrvRepo extends JpaRepository<AppPrivilege, Long>
{
    ...
    @Query("""
            select new lenicorp.eecole.moduleauth.model.dtos.appprivilege.PrvByTypeDTO
            (
                p.prvType.name, p.prvType.uniqueCode, 
                (select new lenicorp.eecole.moduleauth.model.dtos.appprivilege.ReadPrvDTO(p1.privilegeCode, p1.privilegeCode, p1.privilegeName, p1.prvType.name) from AppPrivilege p1 where p1.prvType.uniqueCode = ?1)
            ) 
            from AppPrivilege p where p.prvType.uniqueCode = ?1
             """)
