...
public interface FunctionRepo extends JpaRepository<AppFunction, Long>
{
    ...
    @Query("select f.classeIds from AppFunction f where f.user.email= ?1 and f.fncStatus = 1 and coalesce(f.startsAt, current_date ) <= current_date and coalesce(f.endsAt, current_date) >= current_date")
    Set<Long> getCurrentFncClasseIds(String username);
