package lenicorp.eecole.typemodule.controller.repositories;
...
public interface TypeRepo extends JpaRepository<Type, String>
{
    ...
    @Query("select new lenicorp.eecole.typemodule.model.dtos.ReadTypeDTO(t.uniqueCode, t.typeGroup, t.name, t.status, t.objectFolder) from Type t where t.typeGroup = ?1 and t.status = 'ACTIVE'")
    ...
    @Query("select new lenicorp.eecole.typemodule.model.dtos.ReadTypeDTO( t.uniqueCode, t.typeGroup, t.name, t.status) from Type t order by t.typeGroup, t.uniqueCode, t.objectFolder")
    List<ReadTypeDTO> findAllTypes();

    @Query("select new lenicorp.eecole.typemodule.model.dtos.ReadTypeDTO(s.child.uniqueCode, s.child.typeGroup, s.child.name, s.child.status) from TypeParam s where s.parent.uniqueCode = ?1 order by s.child.name")
